  ...
  <CContainer class="min-vh-100">
    <CRow class="w-100 no-gutters">
      <CCol sm="6" class="text-center text-sm-left">
        <h1 class="mr-sm-4">STORE TYPE MANAGEMENT</h1>
      </CCol>
      <CCol sm="6" class="text-center text-sm-right">
        ...
        <router-link to="/storetype/details/0">
          ...
          >
            ...
            <template v-slot:cell(id)="data">
              <div class="d-flex justify-content-center">
                <router-link :to="'/storetype/details/'+data.item.id">
  ...
  methods: {
    getList: async function() {
      ...
      let data = await this.$callApi(
        "post",
        `${this.$baseUrl}/api/branchType/list`,
        null,
        this.$headers,
        this.filter
      );
    ...
    deleteData: async function(id) {
      if (confirm("Are you sure you want to delete this data?") == true) {
        let data = await this.$callApi(
          "delete",
          `${this.$baseUrl}/api/branchType/removeBranchType/` + id,
          null,
          this.$headers,
          null
        );
